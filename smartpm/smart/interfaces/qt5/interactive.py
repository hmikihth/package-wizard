...
from smart.interfaces.qt5.channels import QtChannels, QtChannelSelector
from smart.interfaces.qt5.mirrors import QtMirrors
from smart.interfaces.qt5.flags import QtFlags
from smart.interfaces.qt5.priorities import QtPriorities, QtSinglePriority
from smart.interfaces.qt5.packageview import QtPackageView
from smart.interfaces.qt5.packageinfo import QtPackageInfo
from smart.interfaces.qt5.legend import QtLegend
from smart.interfaces.qt5.interface import QtInterface, app
...
def compileActions(group, actions, globals):
    ...
    for action in actions:
        ...
        if action[0] == "find": #HACK
            self = globals["self"]
            act.activated.connect(self.toggleSearch)
        if action[0] == "update-selected-channels": #HACK
            self = globals["self"]
            act.activated.connect(self.selectedChannels)
        if action[0] == "update-channels": #HACK
            self = globals["self"]
            act.activated.connect(self.allChannels)
        if action[0] == "rebuild-cache": #HACK
            self = globals["self"]
            act.activated.connect(self.rebuildCache)
        if action[0] == "upgrade-all": #HACK
            self = globals["self"]
            act.activated.connect(self.upgradeAll)
        if action[0] == "exec-changes": #HACK
            self = globals["self"]
            act.activated.connect(self.applyChanges)
        if action[0] == "clear-changes": #HACK
            self = globals["self"]
            act.activated.connect(self.clearChanges)
        if action[0] == "expand-all": #HACK
            self = globals["self"]
            act.activated.connect(self.expandPackages)
        if action[0] == "collapse-all": #HACK
            self = globals["self"]
            act.activated.connect(self.collapsePackages)
        if action[0] == "edit-channels": #HACK
            self = globals["self"]
            act.activated.connect(self.editChannels)
        if action[0] == "edit-mirrors": #HACK
            self = globals["self"]
            act.activated.connect(self.editMirrors)
        if action[0] == "edit-flags": #HACK
            self = globals["self"]
            act.activated.connect(self.editFlags)
        if action[0] == "edit-priorities": #HACK
            self = globals["self"]
            act.activated.connect(self.editPriorities)
        if action[0] == "summary-window": #HACK
            self = globals["self"]
            act.activated.connect(self.showChanges)
        if action[0] == "log-window": #HACK
            self = globals["self"]
            act.activated.connect(self.showLog)
        if action[0] == "legend-window": #HACK
            self = globals["self"]
            act.activated.connect(self.showLegend)
        if action[0] == "about": #HACK
            self = globals["self"]
            act.activated.connect(self.showAbout)
        if action[0] == "quit": #HACK
            self = globals["self"]
            act.activated.connect(app.quit)
        group.addAction(act)
...
class QtInteractiveInterface(QtInterface):
    ...
    def __init__(self, ctrl, argv=None):
        ...
        class ToggleAction(QtWidgets.QAction):
            ...
            def __init__(self, group, name, label):
                QtWidgets.QAction.__init__(self, name, group)
        ...
        self._toolbar = QtWidgets.QToolBar(self._window)
        for TOOL in TOOLBAR:
        ...
        self._searchbar = QtWidgets.QToolBar(self._window)
        self._searchbar.hide()
        self._window.addToolBar(self._searchbar)
       
        label = QtWidgets.QLabel(_("Search:"), self._searchbar)
        label.show()
        self._searchbar.addWidget(label)

        self._searchentry = QtWidgets.QLineEdit(self._searchbar)
        self._searchentry.returnPressed.connect(self.refreshPackages)
        self._searchentry.show()
        self._searchbar.addWidget(self._searchentry)

        button = QtWidgets.QPushButton(self._searchbar)
        button.clicked[()].connect(self.refreshPackages)
        pixmap = getPixmap("crystal-search")
        button.setIcon(QtGui.QIcon(pixmap))
        button.show()
        self._searchbar.addWidget(button)

        buttongroup = QtWidgets.QButtonGroup(self._searchbar)
        #buttongroup.hide()
        
        self._searchname = QtWidgets.QRadioButton(_("Automatic"), self._searchbar)
        self._searchname.setChecked(True)
        self._searchname.clicked[()].connect(self.refreshPackages)
        buttongroup.addButton(self._searchname)
        self._searchbar.addWidget(self._searchname)
        self._searchname.show()
        self._searchdesc = QtWidgets.QRadioButton(_("Description"), self._searchbar)
        self._searchdesc.setChecked(False)
        self._searchdesc.clicked[()].connect(self.refreshPackages)
        ...
        self._splitter = QtWidgets.QSplitter(QtCore.Qt.Vertical, self._window)
        ...
        self._pi.show()
        self._pv.packageSelected.connect(self._pi.setPackage)
        self._pv.packageActivated.connect(self.actOnPackages)
        self._pv.packagePopup.connect(self.packagePopup)
	...
	print "Run: Update selected channels"
        if selected:
	    print "Run: IF"
    ...
    def packagePopup(self, packageview, pkgs, pnt):
        ...
        menu = QtWidgets.QMenu(packageview)
        menu.move(pnt)
	...
	print "vector Run: Edit channels"
    ...
    def showAbout(self):
        ...
        QtWidgets.QMessageBox.about(self._window, "About " + "Smart Package Manager",
