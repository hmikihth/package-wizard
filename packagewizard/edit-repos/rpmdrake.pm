...
our $blackPanther_release = cat_(
    -e '/etc/blackPanther-release' ? '/etc/blackPanther-release' : '/etc/release'
) || '';
chomp $blackPanther_release;
our ($mdk_version) = $blackPanther_release =~ /(\d+\.\d+)/;
our ($branded, %distrib);
$branded = -f '/etc/sysconfig/oem'
    and %distrib = MDK::Common::System::distrib();
our $myname_update = $branded ? N("Software Update") : N("blackPanther OS Update");
